...
/// both shows and drives the view. Build the app with `native({ shell: 'swift', router: true })`.
///
/// ```swift
...
public final class PhazeRouter {
    /// The view a `WebView` shows.
    public let view: WebPage

    /// Serves `dist` at `<scheme>://localhost` and drags the window from the view's
    /// `[data-tauri-drag-region]`, the attribute Tauri's shell honours, so markup moves between
    /// shells. Load the app with `view.load(_:)`.
    ///
    /// `api` is the origin the app's Phaze Transport calls are forwarded to — the view keeps
    ...
    ///
    /// The view navigates only within its own origin, plus any origin in `allow` — the app's dev
    /// server under `phaze native dev`. A link anywhere else opens in the user's browser, and any
    /// other navigation there is refused: the view is the app, not a browser, and the wry shell
    ...
    public init(dist: URL, scheme: String = "app", api: URL? = nil, views: URL? = nil, allow: [URL] = []) {
        ...
        configuration.urlSchemeHandlers[urlScheme] = RequestHandler(
            ...
            forward: api.map { APIForward(origin: $0, viewOrigin: origin) },
            views: views.map { APIForward(origin: $0, viewOrigin: origin) }
        )
        #if os(macOS)
        ...
        // world, so the view's own scripts can neither post `drag` nor alter the listener.
        ...
        #endif
        view = WebPage(configuration: configuration, navigationDecider: allowlist)
    }

    /// Where the view may go: its own origin and the origins the app named. A link elsewhere is
    /// handed to the system browser; anything else — a script's redirect, a form, a frame — is
    /// refused and said so. `about:` is the empty document WebKit starts from.
    @MainActor
    ...
    /// so the router fetches the route's document and swaps the view.
    public var path: String? {
        ...
        set {
            ...
            Task {
                _ = try? await view.callJavaScript(
